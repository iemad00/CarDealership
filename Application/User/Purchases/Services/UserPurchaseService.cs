using Microsoft.EntityFrameworkCore;
using CarDealership.Data;
using CarDealership.Application.User.Purchases.Dtos;
using CarDealership.Application.Common.Dtos;

namespace CarDealership.Services.User;

public class UserPurchaseService : IUserPurchaseService
{
    private readonly ApplicationDbContext _context;
    private readonly ILogger<UserPurchaseService> _logger;

    public UserPurchaseService(ApplicationDbContext context, ILogger<UserPurchaseService> logger)
    {
        _context = context;
        _logger = logger;
    }

    public async Task<Response<CreatePurchaseRequestData>> CreatePurchaseRequestAsync(int userId, CreatePurchaseRequestRequest request)
    {
        try
        {
            var vehicle = await _context.Vehicles.AsNoTracking().FirstOrDefaultAsync(v => v.Id == request.VehicleId && v.IsActive);
            if (vehicle == null)
            {
                return new Response<CreatePurchaseRequestData> { Success = false, Message = "Vehicle not found or inactive" };
            }

            var exists = await _context.PurchaseRequests.AnyAsync(pr => pr.UserId == userId && pr.VehicleId == request.VehicleId && pr.Status == "Pending");
            if (exists)
            {
                return new Response<CreatePurchaseRequestData> { Success = false, Message = "Pending request already exists" };
            }

            var pr = new Models.PurchaseRequest
            {
                UserId = userId,
                VehicleId = request.VehicleId,
                QuotedPrice = vehicle.Price,
                RequestedAt = DateTime.UtcNow,
                Status = "Pending"
            };
            _context.PurchaseRequests.Add(pr);
            await _context.SaveChangesAsync();

            return new Response<CreatePurchaseRequestData> { Success = true, Message = "Purchase request created", Data = new CreatePurchaseRequestData { RequestId = pr.Id } };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error creating purchase request");
            return new Response<CreatePurchaseRequestData> { Success = false, Message = "Failed to create request" };
        }
    }

    public async Task<List<PurchaseHistoryItemDto>> GetPurchaseHistoryAsync(int userId)
    {
        return await _context.PurchaseRequests
            .AsNoTracking()
            .Where(pr => pr.UserId == userId)
            .OrderByDescending(pr => pr.RequestedAt)
            .Select(pr => new PurchaseHistoryItemDto
            {
                Id = pr.Id,
                VehicleId = pr.VehicleId,
                QuotedPrice = pr.QuotedPrice,
                RequestedAt = pr.RequestedAt,
                Status = pr.Status
            })
            .ToListAsync();
    }
}


